...
with st.sidebar:
    ...
    with st.expander("Configuración", expanded=False):
        ...
        if db_upload:
            try:
                df_new = pd.read_excel(db_upload, decimal=',')
                ...
                if col_poza and col_cover:
                    if st.button("📥 Importar/Fusionar Datos", key="btn_imp_db"):
                        for _, row in df_new.iterrows():
                            ...
                            # ROBUST DECIMAL PARSING (Improved)
                            val_raw = row[col_cover]
                            c_val = None
                            
                            # 1. If already numeric (float/int), use directly
                            if isinstance(val_raw, (int, float)):
                                c_val = float(val_raw)
                            # 2. If string, handle comma/point
                            elif isinstance(val_raw, str):
                                clean_val = val_raw.replace(',', '.')
                                c_val = pd.to_numeric(clean_val, errors='coerce')
