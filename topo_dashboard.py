...
import db_manager
from topo_logic import generar_pptx_report
            ...
            for t in ['A', 'B', 'General']:
                ...
                try:
                    ...
                    fig_map = topo_logic.generar_mapa_matplotlib(
                         ...
                         criterio=poza_data.get('Config', {}).get('Criterio', 'Criterio Excon')
                    )
                    if fig_map:
                        img_heat = io.BytesIO()
    ...
    try:
        ...
        for u_file in uploaded_files:
            if u_file.name.endswith('.csv'):
                ...
                # --- AUTO-FIX: DETECT & HANDLE "QUOTED ROWS" FORMAT ---
                # Check if we have columns but data is mostly NaN (pandas parsed row as 1 string)
                if len(temp_df.columns) > 1 and not temp_df.empty:
                    # Check first few rows. If 2nd column is NaN but 1st col has commas/quotes
                    sample = temp_df.head(5)
                    col0 = sample.iloc[:, 0]
                    other_cols = sample.iloc[:, 1:]
                    
                    # Heuristic: >80% NaN in other columns AND first column contains commas
                    is_bad_parse = other_cols.isna().mean().mean() > 0.8
                    first_col_has_commas = col0.astype(str).str.contains(',', na=False).any()
                    
                    if is_bad_parse and first_col_has_commas:
                        st.warning(f"⚠️ Formato inusual detectado en '{u_file.name}' (Filas comillas). Corrigiendo...")
                        u_file.seek(0)
                        # quoting=3 (QUOTE_NONE) tells pandas not to handle quotes, splitting by comma only
                        temp_df = pd.read_csv(u_file, low_memory=False, quoting=3)
                        
                        # CLEANUP: Remove artifacts from QUOTE_NONE
                        # 1. First column might still have leading "
                        if pd.api.types.is_string_dtype(temp_df.iloc[:, 0]):
                            temp_df.iloc[:, 0] = temp_df.iloc[:, 0].astype(str).str.lstrip('"')
                        
                        # 2. Last column might have trailing "
                        if pd.api.types.is_string_dtype(temp_df.iloc[:, -1]):
                            temp_df.iloc[:, -1] = temp_df.iloc[:, -1].astype(str).str.rstrip('"')
                            
                        # 3. Clean escaped quotes inside fields (""EX-134"" -> "EX-134")
                        obj_cols = temp_df.select_dtypes(include=['object']).columns
                        for c in obj_cols:
                            temp_df[c] = temp_df[c].astype(str).str.replace('""', '"', regex=False)
            else:
        ...
        if df_list:
            ...
            # --- AUTO-FIX: NUMERIC COLUMNS (Comma vs Dot) ---
            # Try to convert key columns to numeric if they are object type
            cols_to_check = ['Elevation_m','Z','Cota', 'CellN_m','Norte','Y', 'CellE_m','Este','X']
...
with st.sidebar:
    ...
    with st.expander("Configuración", expanded=False):
                ...
                if col_poza and col_cover:
                    if st.button("📥 Importar/Fusionar Datos", key="btn_imp_db"):
                        for _, row in df_new.iterrows():
                            ...
                            raw_val = str(row[col_cover]).replace(',', '.')
                            c_val = pd.to_numeric(raw_val, errors='coerce')
                            
                            if pd.notna(c_val) and p_val != "NAN":
                                # HEURISTIC: If value < 1.0 (e.g. 0.23), assume Meters and convert to CM (23.0)
                                if 0.0 < c_val < 1.5: 
                                    c_val = c_val * 100.0
                        ...
                        st.success("Registros importados. (Nota: Valores < 1.5 se asumieron Metros y se pasaron a CM)")
                        st.rerun()
                else:
        ...
        # CHANGED DEFAULT: Index 1 ("Criterio Excon")
        criterio_eval = st.selectbox("Seleccione Criterio de Evaluación:", ["Criterio SQM", "Criterio Excon"], index=1, key="sel_criterio_main")
    ...
    for (pid, df_grp), tab in zip(groups, tabs):
        with tab:
            ...
            with t_down:
                    ...
                    # --- PPTX DOWNLOAD (RESTORED) ---
                    pptx_bytes = generar_pptx_report({pid: poza_data})
                    st.download_button(
                        label="Descargar Reporte Presentación (PowerPoint)",
                        data=pptx_bytes,
                        file_name=f"Reporte_Topo_{pid}_{datetime.now().strftime('%Y%m%d')}.pptx",
                        mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",
                        key=f"btn_down_pptx_{pid}"
                    )
